...
export function JoinButton({ inviteCode, isSignedIn }: Props) {
  ...
  async function handleJoin() {
    if (!isSignedIn) {
      ...
      router.push(`/join/${inviteCode}/process`);
    }
  }
  ...
  return (
    <button
      onClick={handleJoin}
      className="w-full text-white py-5 rounded-full font-bold text-lg shadow-xl hover:shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3"
